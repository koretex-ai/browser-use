...
THINK about what the user actually WANTS — the intent behind the words — and how a capable operator would approach it. Translate class phrases into the concrete terms a website can act on (e.g. "decision makers" means concrete titles — CEO, CTO, Managing Director, Founder — the literal phrase must never be typed into a search box; "cheap flights" means specific dates, routes and a sort order). Consider which site or surface fits the goal, what "done" looks like, and the first move. Order the approach by DEPENDENCY: work the source before the destination — gather data before opening the place it will be written to. If SITE PLAYBOOKS are provided, fold their proven routes into the approach.
