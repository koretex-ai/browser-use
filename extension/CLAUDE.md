...
CONSISTENCY LAYER (post first live runs): (1) SCOUT — triage receives a text-only digest of the active tab (URL/title/element labels, same boundary class as rescue payloads) so first plans are grounded, not guessed. (2) `wait_for` step — readiness polling (text appears) replaces blind waits after navigation. (3) Zero-yield harvest FAILS the subtask, routing to the per-subtask rescue ladder instead of costing a scarce checkpoint replan. (4) Budget-aware checkpoint — receives REPLANS REMAINING + collection count; prompt rule: when nearly out, replan only to the deliverable with data in hand (delivered partial beats undelivered perfect). (5) COLLECTION STORE — every extract's list lines are deduped and kept UNTRUNCATED at task level; write steps use `textFrom:"collected"` and the harness inserts all items verbatim at execution time (collected datasets never round-trip through the cloud; the string ledger remains the orchestrator's lossy judgment digest). Harvest queries for data destined to be written must ask for items pre-formatted for the destination. Salvage receives the full collection. Consistency benchmark protocol: `../bench/consistency/PROTOCOL.md`.

RECIPES: saved, parameterized subtask programs (`packages/storage/lib/recipes/`, glue in `agent/recipes.ts`). After a verified plan-mode success, the executed (post-rescue) programs are auto-parameterized by one orchestrator call (`parameterizeRecipe`) — task-specific literals become `{param}` placeholders — and stored locally keyed by site+intent. Triage receives a one-line digest of the library and may answer `mode:"recipe"` with the id and filled params instead of planning (~$0.001 parameter fill vs ~$0.01-0.02 cold compile); the instantiated subtasks run through the normal checkpoint/rescue loop. Self-healing: when a rescue's corrected program succeeds on a recipe-sourced subtask, the fix is written back into the recipe (deterministic reverse substitution of this run's param values). A replan detaches the run from the recipe (indices no longer map). Recipes are DATA — site lore (URLs, labels, quirks) belongs in them, never in harness code. The library doubles as (task, program) training pairs for an eventual local compiler model.
